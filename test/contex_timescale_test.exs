defmodule ContexTimeScaleTest do
  ...
  describe "validity tests" do
    test "Create with non-date domain" do
      assert_raise FunctionClauseError, fn -> _scale = create_timescale({"fred", 10.0}, {0.0, 100.0}) end
    end

    test "Create with non-numeric range" do
      assert_raise FunctionClauseError, fn -> _scale = create_timescale({~U[2015-01-13 13:00:00.056Z], ~U[2015-01-13 13:00:05.000Z]}, {"0.0", 100.0}) end
    end

    test "Mixed DateTime" do
      assert_raise FunctionClauseError, fn -> _scale = create_timescale({~N[2015-01-13 13:00:00.056], ~U[2015-01-13 13:00:05.000Z]}, {0.0, 100.0}) end
    end
  end

  describe "ticks" do
    test "for 5sec timescale" do
      scale = create_timescale({~U[2015-01-13 13:00:00.056Z], ~U[2015-01-13 13:00:05.000Z]}, {0.0, 100.0})
      expected_ticks = [
    ...
    test "for 5sec timescale - NaiveDateTime" do
      scale = create_timescale({~N[2015-01-13 13:00:00.056], ~N[2015-01-13 13:00:05.000]}, {0.0, 100.0})
      expected_ticks = [
    ...
    test "for 10 day timescale" do
      scale = create_timescale({~U[2015-01-01 13:00:00.056Z], ~U[2015-01-10 13:00:05.000Z]}, {0.0, 100.0})
      expected_ticks = [
    ...
    test "for 10 month timescale" do
      scale = create_timescale({~U[2015-01-01 13:00:00.056Z], ~U[2015-09-28 13:00:05.000Z]}, {0.0, 100.0})
      expected_ticks = [
        ~U[2015-01-01 00:00:00Z],
    ...
    test "for 10 month timescale mid month start and end" do
      scale = create_timescale({~U[2015-01-15 13:00:00.056Z], ~U[2015-09-15 13:00:05.000Z]}, {0.0, 100.0})
      expected_ticks = [
        ~U[2015-01-01 00:00:00Z],
        ~U[2015-02-01 00:00:00Z],
        ~U[2015-03-01 00:00:00Z],
    ...
    test "24month timescale aligns to quarter end" do
      scale = create_timescale({~U[2015-01-13 13:07:00.000Z], ~U[2017-01-13 13:17:00.000Z]}, {0.0, 100.0})
  ...
  describe "formatting" do
    ...
    test "5min timescale shows mins and secs" do
      scale = create_timescale({~U[2015-01-13 13:12:00.000Z], ~U[2015-01-13 13:17:00.000Z]}, {0.0, 100.0})

      expected_formatted_ticks = ["12:00", "12:30", "13:00", "13:30", "14:00", "14:30", "15:00", "15:30", "16:00", "16:30", "17:00"]
      actual_formatted_ticks =
        scale
        |> Scale.ticks_domain()
        |> Enum.map(fn tick -> Scale.get_formatted_tick(scale, tick) end)
    ...
    test "10min timescale shows hrs, mins and secs" do
      scale = create_timescale({~U[2015-01-13 13:07:00.000Z], ~U[2015-01-13 13:17:00.000Z]}, {0.0, 100.0})
    ...
    test "24hr timescale shows days & hrs" do
      scale = create_timescale({~U[2015-01-13 13:07:00.000Z], ~U[2015-01-14 13:17:00.000Z]}, {0.0, 100.0})
